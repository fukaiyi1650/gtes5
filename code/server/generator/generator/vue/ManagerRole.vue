  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>管理员角色管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-button type="primary" icon="delete" class="handle-del mr10" @click="delAll" :disabled="this.delData.length===0">批量删除</el-button>
        <el-input v-model="query.managerRoleId" placeholder="管理员角色ID" class="handle-input mr10" />
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" v-loading="isShowloading">
        ...
        <el-table-column prop="managerRoleId" label="管理员角色ID" sortable />
      ...
      <el-dialog title="修改信息" :visible.sync="editDialogVisible" ref="editform" :before-close="confirmClose" v-dialogDrag>
        <el-form :model="form" :rules="editRule" ref="editform">
          <el-form-item label="管理员角色ID" :label-width="formLabelWidth" prop="managerRoleId">
            <el-input v-model="form.managerRoleId" autocomplete="off" />
          </el-form-item>
...
export default {
  data() {
    return {
      ...
      addRule: {
        managerRoleId: [{ required: true, message: "请输入管理员角色ID", trigger: "blur" }],
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除选中的记录吗？", "提示", {
        ...
        axios({
          url: "/api/managerRole/delete",
          method: "get",
          params: { id: row.managerRoleId }
        }).then(res => {
          this.$message({
            type: "success",
            message: "删除成功"
    ...
    delAll() {
      this.$confirm("确认删除选中的记录吗?", "提示", {
        ...
        this.isShowloading = true;
        let ids = this.delData.map(item => item.managerRoleId).toString();
        axios({
          url: "/api/managerRole/delete",
          method: "get",
          params: { id: ids }
