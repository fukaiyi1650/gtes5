  ...
  <div>
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>角色菜单管理</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-button type="primary" icon="delete" class="handle-del mr10" @click="delAll" :disabled="this.delData.length===0">批量删除</el-button>
        <el-input v-model="query.roleMenuId" placeholder="角色菜单ID" class="handle-input mr10" />
      ...
      <el-table :data="tableData" border class="table" ref="multipleTable" @selection-change="handleSelectionChange" v-loading="isShowloading">
        ...
        <el-table-column prop="roleMenuId" label="角色菜单ID" sortable />
      ...
      <el-dialog title="修改信息" :visible.sync="editDialogVisible" ref="editform" :before-close="confirmClose" v-dialogDrag>
        <el-form :model="form" :rules="editRule" ref="editform">
          <el-form-item label="角色菜单ID" :label-width="formLabelWidth" prop="roleMenuId">
            <el-input v-model="form.roleMenuId" autocomplete="off" />
          </el-form-item>
...
export default {
  data() {
    return {
      ...
      addRule: {
        roleMenuId: [{ required: true, message: "请输入角色菜单ID", trigger: "blur" }],
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm("确认删除选中的记录吗？", "提示", {
        ...
        axios({
          url: "/api/roleMenu/delete",
          method: "get",
          params: { id: row.roleMenuId }
        }).then(res => {
          this.$message({
            type: "success",
            message: "删除成功"
    ...
    delAll() {
      this.$confirm("确认删除选中的记录吗?", "提示", {
        ...
        this.isShowloading = true;
        let ids = this.delData.map(item => item.roleMenuId).toString();
        axios({
          url: "/api/roleMenu/delete",
          method: "get",
          params: { id: ids }
